...
|------|-----|
| 박스 없음 (대기 중) | ~0% |
| 화면 변화 없음 | ~0% (픽셀 비교만, 무시할 수준) |
| 배경만 바뀜 (자막 동일) | 갱신 주기마다 0.5~2초간 코어 1개 50~80% |
| 자막 바뀜 (번역 실행) | 갱신 주기마다 0.5~2초간 코어 1개 70~100% |

> EasyOCR이 CPU 모드에서 이미지 1장당 약 0.5~2초 소요됩니다.  
> GPU가 있으면 `config.py`에서 `gpu=True`로 변경 시 10배 이상 빨라집니다.  
